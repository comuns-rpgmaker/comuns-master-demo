...
//----------------------------------------------------------------------------
// 07/07/21 | Version: 1.2.0 | Various improvements and fixes
//----------------------------------------------------------------------------
// 20/11/20 | Version: 1.1.11 | Short note notation compatibility
// 23/10/20 | Version: 1.1.10 | Fixed mirrored moves bug
// 22/09/20 | Version: 1.1.9 | Fixed mirror bug
// 20/09/20 | Version: 1.1.8 | Fixed escape issue
// 20/09/20 | Version: 1.1.7 | Fixed return to home bug
// 12/09/20 | Version: 1.1.6 | Fixed entering action bug
// 12/09/20 | Version: 1.1.5 | Fixed non atb bug
// 12/09/20 | Version: 1.1.4 | Fixed skills in notetag bug
// 12/09/20 | Version: 1.1.3 | Fixed return to base bug
// 10/09/20 | Version: 1.1.2 | Fixed no targets bug
// 06/09/20 | Version: 1.1.1 | Added team battle skill compatibility
// 03/09/20 | Version: 1.1.0 | Added new actions
//----------------------------------------------------------------------------
// 02/09/20 | Version: 1.0.3 | Fixed jump bug
// 01/09/20 | Version: 1.0.2 | Added weapon animations
// 01/09/20 | Version: 1.0.1 | Released on github
// 01/09/20 | Version: 1.0.0 | Released
//----------------------------------------------------------------------------
...
/*:
 * @target MZ
 * @plugindesc [v1.2.0] Akea Animated Battle System 2
...
Akea.BattleSystem.VERSION = [1, 2, 0];
...
Game_Battler.prototype.callAkeaActions = function (actionName, parameters, action, targets) {
    ...
    for (let key in parsedParams){
        id = Number(parsedParams[key]) ? parseInt(parsedParams[key]) : parsedParams[key];
        break;
    }
    switch (actionName) {
        case "Randomize":
...
Game_Akea_Actions.prototype.initialize = function () {
    ...
    this._akeaParsedSkillList = [];
    this._akeaSkillList.forEach((skill, id) => {
        skill = JSON.parse(skill)
        this._akeaParsedSkillList[id] = skill;
    });
...
Game_Akea_Actions.prototype.idToAction = function (id, targets, moveAction) {
    let actionObj = this._akeaParsedSkillList[id - 1];
...
Game_Akea_Actions.prototype.addAkeaSkillActions = function (id, targets, type, moveAction) {
    let actionObj;
    if (typeof id == "string") {
        actionObj = this._akeaParsedSkillList.find(action => action.info === id);
    } else {
        actionObj = this._akeaParsedSkillList[id - 1];
    }
    let action = new Game_Akea_Action();
    let mirror = actionObj.mirror == "true" ? true : false;
...
Game_Battler.prototype.translateSkillActions = function (action, targets, notes, extraHit = false) {
    ...
    BattleManager._spriteset.battlerSprites().forEach(sprite => {
        if (sprite._battler && sprite._battler != this && !targets.includes(sprite._battler)) this._nonTargets.push(sprite._battler);
    });
    let regex;
