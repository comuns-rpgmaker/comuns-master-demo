...
//============================================================================
// Akea Battler Control
//----------------------------------------------------------------------------
// 07/07/21 | Version: 1.0.1 | Fixed reset opacity bug
// 09/07/20 | Version: 1.0.0 | Released
//----------------------------------------------------------------------------
...
/*:
 * @target MZ
 * @plugindesc [v1.0.1] Akea Battler Control
 ...
 *   reset: true/false (optional)
 *   </akeaOpacityUser>
 *       | Battler Opacity - User of the Skill.
 ...
 *   reset: true/false (optional)
 *   </akeaOpacityTarget>
 *       | Battler Opacity - Target of the Skill.
 *       | opacity: how will the opacity be at the end of the movement, 255 is full image, 0 is transparent
 *       | time: the duration of the movement
 *       | reset: at the end resets to default, I recommend this option at the end of all action sequences.
 *   <akeaOpacityNonTargets>
 *   opacity: number 
 *   time: number (optional, 30 is default if not configured)
 *   reset: true/false (optional)
 *   </akeaOpacityNonTargets>
 *       | Battler Opacity - Non-Targets of the Skill.
 *       | opacity: how will the opacity be at the end of the movement, 255 is full image, 0 is transparent
 *       | time: the duration of the movement
 *       | reset: at the end resets to default, I recommend this option at the end of all action sequences.
 */
...
Akea.BattlerControl.VERSION = [1, 0, 1];

if (!Akea.BattleSystem) throw new Error("Akea Battler Control plugin needs the Akea Animated Battle System base.");
if (Akea.BattleSystem.VERSION < [1, 2, 0]) throw new Error("Akea Battler Control [v1.0.1] plugin only works with versions 1.2.0 or higher of the Akea Animated Battle System.");
...
(() => {
    ...
    Sprite_Battler.prototype.updateAkeaBattlerControl = function () {
        ...
        if (this._updateOpacityBCTime > 0) {
            ...
            if (this._updateOpacityBCTime == 0 && this._updateAkeaBCOpacityReset) {
                this.opacity = 255;
            }
        }
    ...
    Sprite_Battler.prototype.updateAkeaBCRotation = function () {
        this.rotation += this._akeaRotateBattlerSpeed;
    ...
    Game_Battler.prototype.callAkeaActions = function (actionName, parameters, action, targets) {
        ...
        if (actionName == "RotateUser") {
            ...
            this._akeaAnimatedBSActions.addCustomAddon(id, targets, actionName, this, action, obj);
        } else if (actionName == "OpacityNonTargets") {
            this._akeaAnimatedBSActions.addCustomAddon(id, targets, actionName, this, action, obj);
        }
    }
    ...
    Sprite_Battler.prototype.manageAkeaActions = function (action) {
        ...
        switch (action.getActionType()) {
            ...
            case "OpacityUser":
                ...
                this._AkeaEndOpacity = (parseInt(obj["opacity"]) - this.opacity) / this._updateOpacityBCTime;
            ...
            case "OpacityTarget":
                ...
                break;
            case "OpacityNonTargets":
                this._battler._nonTargets.forEach(battler => {
                    battler._akeaAnimatedBSActions.addCustomAddon(action.getId(), action.getTargets(), "OpacityUser", this, action.getAction(), action.getObject());
                })
                break;
        }
    }
